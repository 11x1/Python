...
bot_token = "ODgzMzA4MjA4MTEyOTUxMzU3.YTIC3w.JyIeBy7FjXmDNckv9_mGbk5bTzA"
...
async def on_message(message):
    ...
    if message.content.startswith('!test'):
        ...
        msg.set_thumbnail(url="https://nrg.edu.ee/sites/nrg.edu.ee/files/styles/hitsa_core_logo/public/ruutjuurega_0"
                              ".png?itok=dNstk4JL")
        msg.set_footer(
            text=f"Response: {(datetime.datetime.now() - time_start_help)} sec ({(datetime.datetime.now().microsecond - time_start_help_ms) / 1000000} ms)")
        ...
        try:
            ...
            data_send.set_footer(
                text=f"Webelements: {time_delta.microseconds / 1000000} sec | Total: {datetime.datetime.now() - time_finished_start} sec")
